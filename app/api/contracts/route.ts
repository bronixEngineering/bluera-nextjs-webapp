...
export async function GET(req: NextRequest) {
  try {
    ...
    const isTest = isTestParam === "true" ? true : isTestParam === "false" ? false : undefined;

    let query = supabase.from("contracts").select("id,name,address,abi,is_test,created_at").order("created_at", { ascending: false }).limit(1);
