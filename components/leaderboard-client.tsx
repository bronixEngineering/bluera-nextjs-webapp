...
type TabKey = 'allTimeVolume' | 'weeklyVolume' | 'monthlyVolume' | 'netWorth';

function getValueForTab(wallet: WalletStats, tab: TabKey, formatValue: (v: number | null) => string) {
  switch (tab) {
    case 'allTimeVolume':
...
const LeaderboardRows = React.memo(function LeaderboardRows({
  rows,
  tab,
  formatValue,
}: {
  ...
  return (
    <>
      {rows.map((wallet, index) => {
        const currentValue = getValueForTab(wallet, tab, formatValue);
        return (
          <div
          ...
          >
            <div className="flex items-center gap-3 flex-1 min-w-0">
              {/* Rank Badge */}
              <div
                className={`relative w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 transition-colors ${
                  index === 0
              ...
              >
                {index < 3 ? <Medal className="h-4 w-4" /> : <span className="text-xs">#{index + 1}</span>}
              </div>
            ...
            <div className="text-right flex-shrink-0 ml-2">
              <div className="text-sm font-bold">{currentValue}</div>
            </div>
          </div>
        );
      })}
...
export function LeaderboardClient({ initialData, error }: LeaderboardClientProps) {
  ...
  // Precompute each leaderboard once. Switching tabs becomes an instant visibility toggle
  // (no re-ordering the same DOM nodes), which avoids "flicker" in mobile webviews.
  const dailySorted = useMemo(() => sortData(initialData, 'volume_daily'), [initialData]);
  const weeklySorted = useMemo(() => sortData(initialData, 'volume_weekly'), [initialData]);
  const monthlySorted = useMemo(() => sortData(initialData, 'volume_monthly'), [initialData]);
  const netWorthSorted = useMemo(() => sortData(initialData, 'net_worth'), [initialData]);

  const activeTabKey = activeTab as TabKey;
