...
export function LeaderboardClient({ initialData, error }: LeaderboardClientProps) {
  ...
  // Smooth tab switch (cross-fade). In iOS/Base webviews, animating reorder (FLIP) can jitter;
  // cross-fading avoids the "flash/repaint" without complex DOM measurements.
  ...
  const switchTabSmoothly = (next: string) => {
    if (next === activeTab) return;
    setIsTabSwitching(true);
    if (tabSwitchTimerRef.current) window.clearTimeout(tabSwitchTimerRef.current);
    // Small delay so the fade-out is visible before the reorder happens.
    tabSwitchTimerRef.current = window.setTimeout(() => {
      setActiveTab(next);
      // Fade back in shortly after the new list paints.
      window.setTimeout(() => setIsTabSwitching(false), 140);
    }, 90);
  };
  ...
  return (
    <div className="py-6 space-y-4">
      ...
      >
        <button
          onClick={() => switchTabSmoothly('allTimeVolume')}
        ...
        <button
          onClick={() => switchTabSmoothly('weeklyVolume')}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap flex items-center ${
        ...
        <button
          onClick={() => switchTabSmoothly('monthlyVolume')}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap flex items-center ${
        ...
        <button
          onClick={() => switchTabSmoothly('netWorth')}
          className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap flex items-center ${
            activeTab === 'netWorth'
      ...
      <Card className="overflow-hidden">
        <CardContent className="p-0">
          <ScrollArea className="h-[500px]">
            <div
              className={`divide-y transition-opacity duration-200 ${
                isTabSwitching ? 'opacity-60' : 'opacity-100'
              }`}
            >
